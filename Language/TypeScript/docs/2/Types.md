...
書き方についてはすでに基本的な部分は別のところで扱っているので、簡単に特徴をまとめておきます。

`function` は実行可能なので、後ろに `()` をつけると実行可能です。
頑張れば文字列化（関数の中身を文字列にして表示）も出来なくはないですが、ネイティブ実装であったり巨大だと表示しきれないため、数値や文字列と違ってJSONとして保存できなかったりします。
（JavaScriptには一部文字列化可能なデータを保持しておく仕組みがあったりするが、その時`function`は消されるとかする。）

他にもいろいろありますが、とりあえずここではこの程度でまとめておきます。
