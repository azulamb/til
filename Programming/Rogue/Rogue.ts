...
class BitCanvas {
	...
	public toString(f: string = ' ', t: string = '█') {
		return this.canvas.map((v, i) => {
			return (v ? t : f) + ((i + 1) % this.w === 0 ? '\n' : '');
		}).join('');
	}
}
...
export class Dungeon {
	...
	private prepareRoute(route: number, random: Random) {
		...
		switch (type) {
			case 1:
				return this.createRouteV(r[1], r[0], random);
			case 4:
				return this.createRouteV(r[0], r[1], random);
			case 2:
				return this.createRouteH(r[0], r[1], random);
			case 8:
				return this.createRouteH(r[1], r[0], random);
		}
		return [];
	}
	...
	private createRouteV(u: Room, d: Room, random: Random) {
		...
		routes.push(
			{
				x0: v,
				y0: u.roomY() + u.roomHeight(),
				x1: v,
				y1: u.areaY() + u.areaHeight(),
			},
		);
		...
		routes.push(
			{
				x0: Math.min(routes[0].x0, routes[1].x0),
				y0: v,
				x1: Math.max(routes[0].x0, routes[1].x0),
				y1: v,
			},
		);
	...
	private createRouteH(l: Room, r: Room, random: Random) {
		...
		routes.push(
			{
				x0: l.roomX() + l.roomWidth(),
				y0: v,
				x1: l.areaX() + l.areaWidth(),
				y1: v,
			},
		);
		...
		routes.push(
			{
				x0: v,
				y0: Math.min(routes[0].y0, routes[1].y0),
				x1: v,
				y1: Math.max(routes[0].y0, routes[1].y0),
			},
		);
...
class Room {
	...
	public join(room: Room) {
		...
		if (room.parent) {
			room.parent.rooms.forEach((room) => {
				addrooms.push(room);
			});
		}
	...
	public setDrawArea(drawer: (w: number, h: number, x: number, y: number, connect: boolean) => { x?: number; y?: number; w: number; h: number }) {
		const list = this.rooms.map((room, index) => {
			return room.area;
		});
		...
		const area = list.map((area, index) => {
			...
			if (typeof d.x !== 'number') {
				d.x = Math.floor(area.w - area._w) / 2;
			}
			if (typeof d.y !== 'number') {
				d.y = Math.floor(area.h - area._h) / 2;
			}
			area._x = area.x + d.x;
			area._y = area.y + d.y;
			return area;
		...
		area.forEach((_area, a) => {
			for (let b = a + 1; b < area.length; ++b) {
				if (!(_area.x === area[b].x || _area.y === area[b].y)) {
					continue;
				}
	...
	public draw(canvas: BitCanvas) {
		const area = this.rooms.map((room, index) => {
			return room.area;
		});
		area.unshift(this.area);
		area.forEach((area) => {
			if (area._w <= 0 || area._h <= 0) {
				return;
			}
	...
	private init() {
		...
		for (let y = 0; y < this.h; ++y) {
			for (let x = 0; x < this.w; ++x) {
				if (0 < x) {
					this.connect(y * this.w + x - 1, y * this.w + x);
				}
				if (0 < y) {
					this.connect((y - 1) * this.w + x, y * this.w + x);
				}
			}
		}
	}

	public calcRoomNumber(x: number, y: number) {
		return y * this.w + x;
	}
